...
__EXTERN __global__ EXPORT void run_simulation(unsigned long long loops, const unsigned long long seed,  unsigned long long* out) {
  ...
  for(int _loop = 0; _loop < loops; ++_loop) {
    ...
    for (int k = 0; k < T; ++k) {
      /* 3 directions per sublattice. Use rejection: 2 bits (0-3), reject 3. */
      int dir;
      do {
        if (rnd_left _ < 2) {
      ...
      /* Sublattice A ((x+y) even): (+1,0), (-1,0), (0,+1)
         Sublattice B ((x+y) odd):  (+1,0), (-1,0), (0,-1) */
      int sublattice = (pos_x + pos_y) & 1;
      if (dir == 0) { pos_x += 1; }
      else if (dir == 1) { pos_x -= 1; }
      else { pos_y += (sublattice == 0) ? 1 : -1; }
