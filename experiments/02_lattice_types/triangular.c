...
/* 6 directions: (+1,0),(-1,0),(0,+1),(0,-1),(-1,+1),(+1,-1) */
__device__ int DX[6]; /* = {1,-1,0,0,-1,1} -- initialized in run_simulation */
__device__ int DY[6]; /* = {0,0,1,-1,1,-1} */
__EXTERN __global__ EXPORT void run_simulation(unsigned long long loops, const unsigned long long seed,  unsigned long long* out) {
  ...
  /* Initialize direction tables */
  if (TID_LOCAL == 0) {
    DX[0]=1; DX[1]=-1; DX[2]=0; DX[3]=0; DX[4]=-1; DX[5]=1;
  ...
  for(int _loop = 0; _loop < loops; ++_loop) {
    ...
    for (int k = 0; k < T; ++k) {
      /* Each step needs ~2.58 bits (6 directions). Use rejection sampling:
         take 3 bits (0-7), reject 6,7; gives uniform 0-5. */
      int dir;
      do {
        if (rnd_left _ < 3) {
