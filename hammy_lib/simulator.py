...
class Simulator:
    def __init__(
        ...
        if digest:
            ...
            self.calibration_results_cache_key = CalibrationResultsCacheKey(
                ...
                numpy_hash=hash_to_digest(
                    to_int_hash(inspect.getsource(self.python_simulator_function))
                ),
                cffi_hash=hash_to_digest(to_int_hash(self.ffi_source)),
                cuda_hash=hash_to_digest(to_int_hash(self.cuda_source))
                if use_cuda
                else None,
            )
            self.digest = self.calibration_results_cache_key.digest()
        self.calibration_results_cache_file = (
            self.RESULTS_DIR
            / f"{self.experiment.to_folder_name()}_{self.digest}_calibration.json"
        )
        self.simulation_results_cache_file = (
            self.RESULTS_DIR
            / str(self.experiment.to_folder_name())
            / f"{self.digest}_simulation.nc"
        )
    ...
    def dump_calibration_results(self, calibration_results: CalibrationResults) -> None:
        if self.calibration_results_cache_key is None:
            raise ValueError(
                "The simulator was created with a ready digest. Cannot dump calibration results."
            )
    ...
    def run_sequential_calibration(self, force=False) -> CalibrationResults:
        if not force:
            ...
            if cached_calibration_results:
                print(
                    f"Using cached calibration results from {self.calibration_results_cache_file}"
                )
    ...
    def run_parallel_calibration(
        self, force_run_sequential=False, tolerance=25
    ) -> CalibrationResults:
        sequential_results = self.run_sequential_calibration(force=force_run_sequential)
        parallel_results = self.run_parallel_simulations(
            sequential_results, calibration_mode=True
        )
        discrepancies = []
        for platform in parallel_results:
            diff_percent = (
                abs(parallel_results[platform] - sequential_results[platform])
                / sequential_results[platform]
                * 100
            )
            if diff_percent > tolerance:
                discrepancies.append(
                    f"{platform.value}: {diff_percent:.1f}% difference ({parallel_results[platform] - sequential_results[platform]:+d} loops, parallel: {parallel_results[platform]}, sequential: {sequential_results[platform]})"
                )
                print(
                    f"WARNING: {platform.value} calibration differs by {diff_percent:.1f}% ({parallel_results[platform] - sequential_results[platform]:+d} loops, parallel value: {parallel_results[platform]})"
                )
            else:
                print(
                    f"OK: {platform.value} calibration matches within {diff_percent:.1f}% ({parallel_results[platform] - sequential_results[platform]:+d} loops, parallel value: {parallel_results[platform]})"
                )
        if discrepancies:
            raise ValueError(
    ...
    def run_level_simulation(
        ...
        if cached_simulation_results is not None:
            ...
            if (
                calibration_results
                and calibration_results != dataset_calibration_results
            ):
                raise ValueError(
                    "Calibration results do not match cached simulation results."
                )
            calibration_results = dataset_calibration_results
            if not calibration_results:
                raise ValueError(
                    "No calibration results found in cached simulation results."
                )
        ...
        for level in range(current_level + 1, max_level + 1):
            ...
            loops_by_platform = {
                platform: int(calibration_results[platform] * minutes)
                for platform in calibration_results
            }
            current_results.append(self.run_parallel_simulations(loops_by_platform))
        new_results = xr.concat(current_results, dim="level")
        new_results = new_results.assign_coords(
            level=range(current_level + 1, max_level + 1)
        )
        if cached_simulation_results is not None:
            res = xr.concat([cached_simulation_results, new_results], dim="level")
        else:
            res = new_results
        res.attrs["CalibrationResults"] = json.dumps(
            calibration_results_to_plain_dict(calibration_results)
        )
