...
class MachineConfiguration(DictHammyObject):  
  def calculate(self) -> None:
    ...
    self.metadata['total_ram_gb'] = round(float(psutil.virtual_memory().total / (1024**3)), 1)
    ...
    try:
      ...
      if gpu_info and len(gpu_info) >= 2:
        ...
        self.metadata['gpu_memory_gb'] = round(float(gpu_info[1].strip().split()[0]) / 1024, 1)  # Convert MiB to GB
        self.metadata['cuda_cores'] = int(gpu_info[2].strip()) if len(gpu_info) > 2 else None
    except Exception:
      pass
    # Detect CUDA version
    ...
    try:
      ...
      self.metadata['cuda_version'] = cuda_info.split("release")[-1].split(",")[0].strip() \
    ...
    self.metadata['ccompiler'] = MachineConfiguration.get_compiler(self.metadata['os_name'])    
