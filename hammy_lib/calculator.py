...
class Calculator(abc.ABC):
    ...
    def extend_simulation_results(simulation_results: xr.DataArray) -> xr.DataArray:
        ...
        if "TOTAL" in simulation_results.platform:
            raise ValueError(
                "simulation_results is already extended (contains 'TOTAL' platform)"
            )
        # Verify required dimensions exist
        if (
            "level" not in simulation_results.dims
            or "platform" not in simulation_results.dims
        ):
            raise ValueError(
    ...
    def calculate(self) -> xr.DataArray:
        ...
        if (
            "platform" not in self.input.dims
            or "TOTAL" not in self.input.coords["platform"].values
        ):
            raise ValueError(
                "Input must be extended with 'TOTAL' coordinate on 'platform' dimension"
            )
...
class MeanCalculator(Calculator):
    def __init__(
        self,
        simulation_results: xr.DataArray,
        parallelizable_dimensions: List[str] = [],
    ):
...
class ArgMaxCalculator(Calculator):
    def __init__(
        self, simulation_results: xr.DataArray, independent_dimensions: List[str] = []
    ):
        extra_dims = [
            dim
            for dim in simulation_results.dims
            if dim not in ["platform", "level"] + independent_dimensions
        ]
        if len(extra_dims) == 0:
            raise ValueError(
                "Input must have at least one dimension besides platform, level and independent_dimensions"
            )
...
class RandomnessTestCalculator(Calculator):
    def __init__(
        self, simulation_results: xr.DataArray, theoretical_values: xr.DataArray
    ):
...
class SCSCalculator(Calculator):
    ...
    def get_approximation(width_in_steps, data):
        ...
        inv_y[inv_y == 0] = 0.5
        inv_y = 1 / inv_y
        W = sparse.dia_matrix(((1.0 / y).reshape(1, -1), np.array((0,))), shape=(M, M))
        ...
        solver = scs.SCS(
            data, SCS_CONSTANTS.cone, eps_abs=1e-9, eps_rel=1e-9, verbose=False
        )
        return solver.solve()
