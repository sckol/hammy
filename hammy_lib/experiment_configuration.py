...
class ExperimentConfiguration(DictHammyObject):
    ...
    @property
    def id(self) -> str:
        return f"{self.experiment.id}_{self.machine_configuration.digest}_experimentconfiguration"
    ...
    def run_single_simulation(self, platform: SimulatorPlatforms, loops: int, calibration_mode=False) -> xr.DataArray | float:
        ...
        result = self.experiment.run_single_simulation(self.seed, platform, loops, calibration_mode)
        self.seed += 1
        return result
    ...
    def run_simulation_thread(self, thread_id: int, loops_by_platform: CalibrationResults, calibration_mode=False) -> xr.DataArray | float:
        ...
        return self.experiment.run_single_simulation(self.seed + thread_id, platform, loops_by_platform[platform], calibration_mode)
    ...
    def run_parallel_simulations(self, loops_by_platform: CalibrationResults, calibration_mode=False) -> xr.DataArray | CalibrationResults:
        res = self.pool.map(partial(self.run_simulation_thread, loops_by_platform=loops_by_platform, calibration_mode=calibration_mode), list(range(self.threads)))
        if calibration_mode:
