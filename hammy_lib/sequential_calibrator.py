...
from .calibration_results import CalibrationResults, calibration_results_from_plain_dict, calibration_results_to_plain_dict
from .experiment_configuration import ExperimentConfiguration
from .simulator_platforms import SimulatorPlatforms

class SequentialCalibrator(DictHammyObject):
    def __init__(self, experiment_configuration: ExperimentConfiguration, digest: str = None) -> None:
    ...
    def calculate(self) -> None:
        ...
        platforms += [SimulatorPlatforms.CUDA] if self.experiment_configuration.use_cuda else []
        for platform in platforms:    
          results[platform] = self.run_single_calibration(platform)
        self.metadata['sequential_calibration_results'] = json.dumps(calibration_results_to_plain_dict(results))
